...
class LineClient(LineAPI):
    ...
    def __init__(self, id=None, password=None, authToken=None, is_mac=True, com_name="alfathdirk"):
        ...
        # self._headers['User-Agent']         = 'Mozilla/5.0 (Macintosh; Intel Mac OS X 10_10_5) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/44.0.2403.157 Safari/537.36'
        self._headers['User-Agent']         = user_agent
        self._headers['X-Line-Application'] = app
        # self._headers['X-Line-Application'] = 'CHROMEOS  1.0.5   Chrome_OS   1'
        if authToken:
    ...
    def longPoll(self, count=50, debug=False):
        ...
        for operation in operations:
            ...
            if operation.type == OT.END_OF_OPERATION:
                ...
                try:
                    NameData = self._getContacts([operation.param2])
                    lowerName = NameData[0].displayName.lower()
                    if 'bot' in lowerName:
            			self._kickoutFromGroup(0,operation.param1,operation.param2)
                    if operation.param2 in botlist:
                        self._kickoutFromGroup(0,operation.param1,botlist)
                except:
        		    print "gagal"
                ...
                try:
                    NameData = self._getContacts([operation.param3])
                    lowerName = NameData[0].displayName.lower()
                    if 'bot' in lowerName:
                        self._cancelGroupInvitation(operation.param1,operation.param3)
                    if operation.param3 in botlist:
                        self._cancelGroupInvitation(operation.param1,botlist)
                except:
                    print "cannot!"
